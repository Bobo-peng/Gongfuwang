...
<body>
<form class="sale-info" role="form" method="post" action="TotalSaleServlet">
...
<option value="可乐">可乐</option>
	...
	<input type="date"  name="day2" value="2018-06-20"  onfocus="WdatePicker({skin:'default',dateFmt:'yyyy-MM-dd',isShowClear:false,readOnly:true})">
		...
		&nbsp;
		&nbsp;
	<input type="submit"  class="search_click" value="销售查询"/>
	</form>
<style type="text/css">
...
<c:forEach items="${list}" var="totalsale">  
    <tr>  
         <td>${totalsale.getShopName()}</td>  
		
		<td>${totalsale.getProductName()}</td>  
        <td>${totalsale.getPrice()}</td>   
     
    	<td>${totalsale.getTotalSum()}</td>
    	<td>${totalsale.getSale()}</td>   
     
    	<td>${totalsale.getSaleDate()}</td>  
    </tr>  
</c:forEach> 
 </table>
</body>
